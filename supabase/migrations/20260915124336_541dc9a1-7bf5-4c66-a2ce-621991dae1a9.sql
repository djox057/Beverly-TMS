...
BEGIN
  ...
  -- internal_load_number is globally unique, so if the per-company candidate is
  -- already taken by another company, continue from the global maximum instead.
  IF EXISTS (
    SELECT 1 FROM orders WHERE internal_load_number = next_load_number::text
  ) THEN
    SELECT COALESCE(MAX(
      CASE
        WHEN internal_load_number ~ '^\d+'
