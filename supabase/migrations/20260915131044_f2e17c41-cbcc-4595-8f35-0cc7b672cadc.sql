DROP INDEX IF EXISTS public.orders_internal_load_number_unique;

CREATE UNIQUE INDEX IF NOT EXISTS orders_company_internal_load_number_unique
  ON public.orders (company_id, internal_load_number)
...
BEGIN
  ...
  suffix := CASE
    WHEN company_name_val ILIKE '%lale transport%' THEN 'LT'
    WHEN company_name_val ILIKE '%jones freight%' THEN 'JF'
  ...
  -- internal_load_number is unique per company, so only skip numbers already
  -- used inside this company.
  WHILE EXISTS (
    SELECT 1 FROM orders
    WHERE company_id = company_uuid
      AND internal_load_number = next_load_number::text
  ) LOOP
    next_load_number := next_load_number + 1;
  END LOOP;
