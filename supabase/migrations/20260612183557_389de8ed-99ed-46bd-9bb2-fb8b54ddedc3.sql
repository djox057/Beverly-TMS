...
-- Ensure trigram extension and indexes exist for substring search
...
-- Lightweight: returns only matching order ids (<=p_limit). RLS still applies.
...
AS $function$
  ...
  matched AS (
    SELECT id, created_at FROM exact_matches
    UNION ALL
    SELECT id, created_at FROM substring_matches
  )
  SELECT COALESCE(array_agg(id ORDER BY created_at DESC), ARRAY[]::uuid[])
  FROM matched;
$function$;

GRANT EXECUTE ON FUNCTION public.search_orders_ids(text, text, uuid, uuid, uuid, int)
  TO authenticated, service_role;

-- Hydrate full payload from a list of order ids. RLS applies via SECURITY INVOKER.
CREATE OR REPLACE FUNCTION public.search_orders_hydrate(p_ids uuid[])
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $function$
  WITH matched AS (
    SELECT o.* FROM public.orders o WHERE o.id = ANY(p_ids)
  )
...
GRANT EXECUTE ON FUNCTION public.search_orders_hydrate(uuid[])
  TO authenticated, service_role;
