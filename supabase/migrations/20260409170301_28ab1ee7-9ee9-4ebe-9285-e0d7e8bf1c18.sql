...
ALTER TABLE public.assignment_history
  ADD COLUMN IF NOT EXISTS dispatcher_name_snapshot text,
  ADD COLUMN IF NOT EXISTS old_dispatcher_name_snapshot text,
  ADD COLUMN IF NOT EXISTS changed_by_name_snapshot text;

-- Update get_assignment_history to use COALESCE fallbacks
...
RETURNS TABLE(
  ...
  dispatcher_id uuid, old_dispatcher_id uuid,
  ...
  dispatcher_name text, old_dispatcher_name text
)
LANGUAGE plpgsql
SECURITY DEFINER
