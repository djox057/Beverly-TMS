...
// Slim column list for Analytics page - see get-all-locked-orders for rationale.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[get-all-unlocked-orders] limit=${limit}, offset=${offset}, fields=${fields}`);
    ...
    // Step 3: Batch fetch child relations.
    // Analytics mode skips order_files + order_transfers entirely.
    ...
    const truckIds = fields === "analytics"
      ? collectUniqueIds(allOrders, "truck_id")
      : collectUniqueIds(allOrders, "truck_id", "original_truck_id");
    const driverIds = fields === "analytics"
      ? collectUniqueIds(allOrders, "driver1_id", "driver2_id")
      : collectUniqueIds(allOrders, "driver1_id", "driver2_id", "original_driver1_id", "original_driver2_id");
    const brokerIds = fields === "analytics" ? [] : collectUniqueIds(allOrders, "broker_id");
    const companyIds = collectUniqueIds(allOrders, "company_id", "booked_by_company_id");
    const trailerIds = fields === "analytics" ? [] : collectUniqueIds(allOrders, "trailer_id", "original_trailer_id");
