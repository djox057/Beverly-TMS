...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("[get-all-locked-orders] Starting bulk fetch...");
    ...
    // Step 2: Fetch ALL locked orders in batches (bypass 1000 row limit)
    ...
    while (true) {
      ...
      if (batchError) {
        console.error(`[get-all-locked-orders] Batch error at offset ${offset}:`, batchError);
        throw batchError;
      }
      ...
      console.log(`[get-all-locked-orders] Fetched batch: ${batch.length}, total: ${allOrders.length}`);
    ...
    console.log(`[get-all-locked-orders] Completed: ${allOrders.length} orders in ${fetchTime}ms`);

    // Verify data integrity
    if (totalCount !== null && allOrders.length !== totalCount) {
      console.warn(`[get-all-locked-orders] Count mismatch: expected ${totalCount}, got ${allOrders.length}`);
    }
