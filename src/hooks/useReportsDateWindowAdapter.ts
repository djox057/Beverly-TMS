  ...
  const transformedData = useMemo(() => {
    ...
    for (const driverId of driverIds) {
      ...
      group.trucks.push({
        ...
        note: note?.note || "",
        lastEdit: (() => { const d = note?.updated_at ? new Date(note.updated_at) : new Date(); return d.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false }); })(),
        editDate: (() => { const d = note?.updated_at ? new Date(note.updated_at) : new Date(); return `${String(d.getMonth() + 1).padStart(2, '0')}/${String(d.getDate()).padStart(2, '0')}`; })(),
