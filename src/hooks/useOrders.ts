      ...
      return queryWithTimeout(async () => {
        ...
        const transformOrder = (order: any) => {
          ...
          return {
            ...
            totalDriverPay: (order.driver_price || 0) + (order.detention_driver || 0) + (order.layover_driver || 0) - (order.late_fee_driver || 0) - (order.no_tracking_fee_driver || 0) - (order.wrong_address_fee_driver || 0) + (order.tonu_driver || 0) + (order.other_charges_driver || 0),
            ...
            totalFreightAmount: (order.freight_amount || 0) + (order.detention || 0) + (order.layover || 0) + (order.extra_stop || 0) + (order.lumper || 0) + (order.tonu || 0) + (order.other_charges || 0) - (order.late_fee || 0) + (order.escort_fee_broker_paid ? (order.escort_fee || 0) : 0),
            notes: order.notes || '',
