    ...
    queryFn: async () => {
      ...
      const transformedOrders = data.map((order: any) => {
        ...
        return {
          ...
          totalDriverPay: (order.driver_price || 0) + (order.detention_driver || 0) + (order.layover_driver || 0) + (order.extra_stop_driver || 0) + (order.lumper_driver || 0) + (order.late_fee_driver || 0) + (order.tonu_driver || 0),
