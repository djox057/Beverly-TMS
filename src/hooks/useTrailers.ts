...
export const useTrailers = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: allTrailers, error } = await supabase
        .from('trailers')
        .select('*')
      ...
      console.log(`✅ Total trailers fetched: ${allTrailers?.length || 0}`);
      ...
      if (trucksError) {
        ...
        return (allTrailers || []).map(trailer => ({ ...trailer, trucks: [] }));
      }
      ...
      const trailersWithTrucks = (allTrailers || []).map(trailer => ({
        ...trailer,
        trucks: trucksByTrailerId.get(trailer.id) || []
      }));
      
      return trailersWithTrucks;
    },
