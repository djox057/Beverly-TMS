import { TRUCK_DOCUMENT_TYPES, getTruckDocumentTypeById } from "./truckDocumentKeywords";

/**
 * Concept-based search over truck files.
 *
 * A query is resolved to document concepts through per-document alias phrases
 * (so "cab card" -> Registration, "fuel tax" -> IFTA Licence). Files are then
...
const DOC_ALIASES: Record<string, string[]> = {
  annual_dot: ["annual dot", "annual inspection", "dot inspection", "annual", "dot", "inspection", "annual dot inspection"],
  lease_agreement: ["lease agreement", "lease", "truck lease", "equipment lease", "agreement"],
  registration_affirmation: ["registration affirmation", "affirmation", "affirmation of registration", "reg affirmation"],
  ky_permit: ["ky permit", "kentucky permit", "kentucky", "kyu", "ky number", "ky"],
  ny_permit: ["ny permit", "new york permit", "new york", "hut", "ny hut", "highway use tax", "ny"],
...
const FILE_CONCEPTS: Array<{ aliases: string[]; terms: string[] }> = [
  ...
  {
    aliases: ["title", "vehicle title", "mso"],
    terms: ["title", "vehicle title", "mso"],
  },
  {
...
const resolveDocIds = (queryTokens: string[]): Set<string> => {
  ...
  TRUCK_DOCUMENT_TYPES.forEach((doc) => {
...
export interface SearchableTruckFile {
  id: string;
  file_name: string;
...
export interface TruckFileSearchHit<T extends SearchableTruckFile> {
  file: T;
  score: number;
}

export const searchTruckFiles = <T extends SearchableTruckFile>(
  files: T[],
  query: string
): TruckFileSearchHit<T>[] => {
  ...
  const hits: TruckFileSearchHit<T>[] = [];

  files.forEach((file) => {
    const nameTokens = tokenize(file.file_name);
    const keywordTokens = (file.keywords || []).flatMap((k) => tokenize(k));
    const fileDocId = (file.keywords || []).find((k) => getTruckDocumentTypeById(k));
