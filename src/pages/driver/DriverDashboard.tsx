...
export default function DriverDashboard() {
  ...
  const totalEarnings = completedThisMonth.reduce((sum, order) => sum + (Number(order.driver_price) || 0), 0);

  return (
    <div className="min-h-screen bg-background p-4 pb-20">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-foreground mb-1">Welcome, {data?.driver?.name}</h1>
        <p className="text-sm text-muted-foreground">{data?.truck?.truck_number || "No truck assigned"}</p>
      </header>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <Card className="border-primary/20">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{completedThisMonth.length} loads completed</p>
          </CardContent>
        </Card>
      </div>
