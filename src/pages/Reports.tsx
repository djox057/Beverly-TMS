                            ...
                            <PopoverContent className="w-80 z-[102]">
                              <div className="space-y-2 text-sm">
                                ...
                                <div className="space-y-3">
                                  <div>
                                    ...
                                    {stop.arrived_at && <p className="ml-8 text-green-600 dark:text-green-400 font-medium text-xs">
                                        ✓ Arrived: {formatDateTime(stop.arrived_at, "MM/dd, HH:mm")}
                                      </p>}
                                    <p className="ml-4">
                                      • <strong>Documents:</strong> {formatDocuments(order.loadDetails.documents)}
                                    </p>
                                    {order.loadDetails.notes !== "—" && <p className="ml-4 text-sm font-bold">
                                        • <strong>Notes:</strong> {order.loadDetails.notes}
                                      </p>}
                                  </div>
                                </div>
                                {stop.id && !stop.arrived_at && <div className="space-y-2 mt-2">
                                    {/* Going to Delivery button */}
                                    {shouldShowGoingToDelivery(order, stop) && <Button size="sm" onClick={() => {
                              ...
                              toast({
                                title: "Going to Delivery"
                                    ...
                                    {shouldShowAtDelivery(order, stop) && <Button size="sm" onClick={() => {
              ...
              {pickupOnlyOrders.length > 0 || sameDayOrders.length > 0 ? <div className="space-x-0.5 flex-1 p-0 overflow-hidden flex flex-row">
                            ...
                            <PopoverContent className="w-80 z-[102]">
                              <div className="space-y-2 text-sm">
                                ...
                                {stop.id && !stop.arrived_at && <div className="space-y-2 mt-2">
                                    ...
                                    {shouldShowGoingToPickup(order, stop) && <Button size="sm" onClick={() => {
                              ...
                              toast({
                                title: "Going to Pickup"
                                    ...
                                    {shouldShowAtPickup(order, stop) && <Button size="sm" onClick={() => {
                              updatePickupDropArrival.mutate({
                                pickupDropId: stop.id
                              });
                              toast({
                ...
                return <div key={`same-day-${order.id}-${idx}`} className={`${cellColor} border rounded relative flex flex-col px-1 py-0.5 ${totalCellsOnDay === 1 ? "flex-1" : "shrink-0"} h-full`} style={totalCellsOnDay > 1 ? {
                  width: `${100 / totalCellsOnDay}%`
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="ghost" size="sm" className={`absolute top-[7%] ${isToday ? "right-[7%]" : "right-[1.5%]"} h-2.5 w-2.5 p-0 hover:bg-background/20`}>
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="space-y-1">
                                ...
                                {order.loadDetails.allPickupStops && order.loadDetails.allPickupStops.length > 0 && <>
                                    ...
                                    {order.loadDetails.allPickupStops.map((pickup, pIdx) => <div key={`pickup-${pIdx}`}>
                                        <p className="ml-4">
                                          - {pickup.address}, {pickup.city}, {pickup.state} {pickup.zipCode} at{" "}
                                          {formatDateTime(pickup.datetime, "MM/dd, HH:mm")}
                                        </p>
                                        {pickup.arrived_at && <p className="ml-6 text-green-600 dark:text-green-400 font-medium text-xs">
                                ...
                                {order.loadDetails.allDeliveryStops && order.loadDetails.allDeliveryStops.length > 0 && <>
                                      ...
                                      {order.loadDetails.allDeliveryStops.map((delivery, dIdx) => <div key={`delivery-${dIdx}`}>
                                          <p className="ml-4">
                                            - {delivery.address}, {delivery.city}, {delivery.state} {delivery.zipCode}{" "}
                                            at {formatDateTime(delivery.datetime, "MM/dd, HH:mm")}
                                          </p>
                                          {delivery.arrived_at && <p className="ml-6 text-green-600 dark:text-green-400 font-medium text-xs">
                                ...
                                {order.loadDetails.notes !== "—" && <p className="text-sm font-bold">
                                    • <strong>Notes:</strong> {order.loadDetails.notes}
                                  </p>}
                              </div>
                            </div>
                    ...
                    tableLayout: "auto"
                        ...
                        return <React.Fragment key={truck.id}>
                                    <tr className={truckIndex % 2 === 0 ? "bg-card" : "bg-muted/20"}>
                                      <td className={`border-r border-b-[6px] border-gray-400 px-2 py-1 text-xs font-medium ${shouldShowDrugTestUI ? "cursor-pointer hover:opacity-80" : ""}`} style={{
                              width: "77px",
                              minWidth: "77px",
                              maxWidth: "77px",
                              ...drugTestStyle
