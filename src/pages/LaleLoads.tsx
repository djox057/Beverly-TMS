...
const LaleLoads = () => {
  ...
  const { data: allUserProfiles } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("full_name, email").order("full_name");
  ...
  const { data: allBookedByFromOrders } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return ((data || []) as Array<{ booked_by: string }>).map((r) => r.booked_by).filter(Boolean);
    },
    ...
    () =>
      shouldFilterByUser
        ? {
            ...
            bookedByCompanyId: LALE_COMPANY_ID,
          }
        : {
            bookedBy: null,
    ...
    if (shouldRestore === "true") {
      ...
      if (savedState) {
        try {
          ...
          setCompanyFilter(LALE_COMPANY_NAME);
  ...
  useEffect(() => {
    console.log(
      "🔵 [Orders Page] Current page orders changed! Count:",
      currentPageOrdersFromHook?.length,
      "Page:",
      currentPage,
    );
  ...
  const serverFilters = useMemo(() => {
    ...
    const companyId =
      companyFilter !== "all-companies" ? companies?.find((c) => c.name === companyFilter)?.id : undefined;
    ...
    const loadNumberSuffix =
      truckCompanyFilter !== "all-truck-companies" ? getCompanySuffix(truckCompanyFilter) || undefined : undefined;
    ...
    const truckId = truckFilter !== "all-trucks" ? trucks?.find((t) => t.truck_number === truckFilter)?.id : undefined;

    const driverId = driverFilter !== "all-drivers" ? drivers?.find((d) => d.name === driverFilter)?.id : undefined;

    const brokerId = brokerFilter !== "all-brokers" ? brokerFilter : undefined;
  ...
  useEffect(() => {
    ...
    if (!loadedPages.has(currentPage)) {
      ...
      requestPage(currentPage).catch((err) => console.error(`[Orders] Failed to load page ${currentPage}:`, err));
    }
  }, [
    currentPage,
    loadedPages,
  ...
  const selectedByCompany = selectedOrders.reduce(
    (acc, order) => {
  ...
  const paginatedOrders =
    hasActiveFilter || isActiveSearch
  ...
  const uniqueCompanies = (companies || [])
    ...
    .filter(Boolean)
    .sort();
  const uniqueTruckCompanies = (companies || [])
    .map((c: any) => c.name)
    .filter(Boolean)
    .sort();
  ...
  const uniqueTrucks = (trucks || [])
    .map((t: any) => t.truck_number)
    .filter(Boolean)
    ...
    <div className="h-full w-full">
      <div className="space-y-4 md:space-y-6 p-4 md:p-6 max-w-none">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h1 className="text-2xl md:text-3xl font-semibold text-foreground">LALE Loads</h1>
        ...
        <Card className="w-fit min-w-full">
          <CardHeader>
            <div className="flex items-center justify-between gap-4">
              <div className="relative shrink-0">
                <CardTitle className="shrink-0 whitespace-nowrap">All LALE Loads</CardTitle>
              ...
              <ScrollArea className="w-full">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-4 pb-4">
                  ...
                  />
                   ...
                   {/* Company filter locked to Lale Transport LLC on this page */}
                  ...
                  <Combobox
                    ...
                    searchPlaceholder="Search by name or MC#..."
                    options={[
                      {
                        value: "all-brokers",
                        label: "All Brokers",
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                              {(() => {
                                ...
                                const byCompany = notInvoicedOrders.reduce(
                                  (acc, order) => {
                                    const company = order.bookedByCompanyName || "Unknown";
                                    if (!acc[company]) {
                                      acc[company] = {
          ...
          <CardContent className="p-0">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-[140px] min-w-[140px] max-w-[140px] whitespace-nowrap">
                      Pickup City
                    </TableHead>
                    <TableHead className="w-[100px] min-w-[100px] max-w-[100px] whitespace-nowrap">
                      Delivery Date
                    ...
                    paginatedOrders.map((order, index) => {
                      ...
                      return (
                        <TableRow key={order.id} className={`h-16 ${rowClassName}`}>
                          ...
                          <TableCell className="w-20">
                            <div className="flex items-center gap-1">
                              ...
                              {formatInternalLoadNumber(
                                ...
                                order.companyName || order.truckCompanyName,
                              )}
                            </div>
                          </TableCell>
                          <TableCell className="w-32 p-0">
                            <div className="h-full p-4">{formatDateNoTimezone(order.pickupDate)}</div>
                          </TableCell>
                          ...
                          <TableCell className="w-32 p-0">
                            <div className="h-full p-4">{formatDateNoTimezone(order.deliveryDate)}</div>
                          </TableCell>
                          <TableCell className="w-40 p-0">
                            <div className="h-full p-4 line-clamp-2">
                              {order.deliveryCity}
                          ...
                          <TableCell className="w-24 text-center">
                            <div className="flex gap-1 flex-wrap justify-center">
                              {(() => {
                                const deliveryCount =
                                  order.pickup_drops?.filter((pd: any) => pd.type === "delivery").length || 1;
                                const podCount = order.podFiles?.length || 0;
            ...
            {/* Pagination Controls */}
            {effectiveTotalCount > ORDERS_PER_PAGE && (
              <div className="flex items-center justify-between px-6 py-4 border-t">
                <div className="text-sm text-muted-foreground">
                  Showing {(currentPage - 1) * ORDERS_PER_PAGE + 1} to{" "}
                  {Math.min(currentPage * ORDERS_PER_PAGE, effectiveTotalCount)} of {effectiveTotalCount} loads
                </div>
                <Pagination>
                  <PaginationContent>
                    <PaginationItem>
            ...
            <div className="space-y-2 text-sm">
              ...
              {Object.keys(selectedByCompany).length > 0 && (
                <div className="border-t pt-2 mt-2">
                  ...
                  {Object.entries(selectedByCompany).map(
                    ([company, data]: [
                      string,
                      {
                        count: number;
        ...
        >
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleConfirmPaidChange}>Confirm</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        {/* Invoiced Confirmation Dialog */}
        <Dialog
          open={invoicedConfirmDialogOpen}
          onOpenChange={(open) => {
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {pendingInvoicedOrder?.currentInvoiced
                  ? "Are you sure you want to mark this load as not invoiced?"
                  : "Are you sure you want to mark this load as invoiced?"}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
              ...
              <Button onClick={handleConfirmInvoicedChange}>Confirm</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={invoiceWarningDialogOpen} onOpenChange={setInvoiceWarningDialogOpen}>
          <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Some files had issues during invoice generation. See details below.</DialogDescription>
            </DialogHeader>
                    ...
                    {skippedWarnings.length > 0 && (
                      <div>
                        ...
                        {skippedWarnings.map((warning, idx) => (
                          <div
                            key={`skip-${idx}`}
                            className="border border-destructive/30 rounded-lg p-3 bg-destructive/5 mb-2"
                          >
                    ...
                    {fallbackWarnings.length > 0 && (
                      <div>
                        ...
                        {fallbackWarnings.map((warning, idx) => (
                          <div
                            key={`fb-${idx}`}
                            className="border border-yellow-500/30 rounded-lg p-3 bg-yellow-500/5 mb-2"
                          >
                            <div className="font-medium text-sm mb-2">Invoice: {warning.invoice}</div>
                            <ul className="text-sm text-muted-foreground space-y-1">
                              {warning.files.map((file, fileIdx) => (
