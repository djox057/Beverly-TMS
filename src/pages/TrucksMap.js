...
const FLEET_QUERY_KEY = ["trucks-map-fleet"];
...
async function fetchFleetMapData() {
    ...
    const driverIds = Array.from(new Set(truckList
    ...
    const [driversRes, companiesRes] = await Promise.all([
        driverIds.length
            ? supabase
                .from("drivers")
                .select("id, name, company_id, dispatcher_id, hos_drive_minutes, hos_shift_minutes, hos_break_minutes, hos_cycle_minutes, home_latitude, home_longitude, home_city, home_state")
                .in("id", driverIds)
    ...
    const dispatcherIds = Array.from(new Set((driversRes.data || [])
    ...
    const { data: dispatchersRes, error: dispatchersErr } = dispatcherIds.length
        ...
        : { data: [], error: null };
    if (dispatchersErr)
        throw dispatchersErr;
    const dispatcherMap = new Map((dispatchersRes || []).map((p) => [
        p.user_id || p.id,
        p.full_name || "",
    ]));
    ...
    if (truckIds.length) {
        ...
        const pageSize = 1000;
        const chunks = [];
        for (let i = 0; i < truckIds.length; i += 50)
            chunks.push(truckIds.slice(i, i + 50));
        const fetchChunkOrders = async (ids) => {
            const chunkOrders = [];
            let page = 0;
            while (true) {
                ...
                const res = await supabase
                    .from("orders")
                    .select("id, truck_id, internal_load_number, broker_load_number, pickup_datetime, canceled, notes, pickup_drops(id, order_id, type, sequence_number, address, city, state, zip_code, latitude, longitude, datetime, arrived_at), order_files(order_id, file_category)")
                    .in("truck_id", ids)
                    .eq("canceled", false)
...
function pickCurrentOrder(allOrders) {
    if (!allOrders.length)
        return null;
    ...
    const now = Date.now();
    const hasFile = (order, category) => order.order_files?.some((f) => f.file_category === category) || false;
    const pickupTime = (order) => order.pickup_datetime ? new Date(order.pickup_datetime).getTime() : Infinity;
    ...
    return ([...startedOpenOrders].reverse().find((order) => hasFile(order, "BOL")) ||
...
export default function TrucksMap() {
    ...
    const canUseIndividual = isDispatch || (isAfterhours && afterhoursDriverIds.length > 0);
    const [individualOnly, setIndividualOnly] = useState(isDispatch);
    useEffect(() => {
    ...
    const trucksWithData = useMemo(() => {
        if (!fleet)
            return [];
        const locByTruckId = new Map();
        const locByNumber = new Map();
        for (const l of locations) {
            if (l.truck_id)
                locByTruckId.set(l.truck_id, l);
            if (l.truck_number)
        ...
        return fleet.truckList
            ...
            if (current) {
                ...
                const hasBOL = current.order_files?.some((f) => f.file_category === "BOL") || false;
                const hasPOD = current.order_files?.some((f) => f.file_category === "POD") || false;
                currentOrder = {
                    id: current.id,
                    loadNumber: formatInternalLoadNumber(current.internal_load_number, companyName),
            ...
            if (currentOrder && loc?.latitude != null && loc?.longitude != null) {
                ...
                if (nextLat != null && nextLng != null) {
                    milesAway = Math.round(haversineMiles({ lat: loc.latitude, lng: loc.longitude }, { lat: nextLat, lng: nextLng }) * 1.3);
                }
            }
            return {
                id: t.id,
                ...
                dispatcherName: fleet.dispatcherMap.get(driver1?.dispatcher_id || "") || null,
    ...
    const companyOptions = useMemo(() => {
        const map = new Map();
        for (const t of trucksWithData) {
            if (t.companyId && t.companyName)
    ...
    const dispatcherOptions = useMemo(() => {
        const map = new Map();
        for (const t of trucksWithData) {
            if (t.dispatcherId && t.dispatcherName)
                map.set(t.dispatcherId, t.dispatcherName);
        }
        return Array.from(map.entries())
            .map(([value, label]) => ({ value, label }))
            .sort((a, b) => a.label.localeCompare(b.label));
    ...
    const filteredTrucks = useMemo(() => {
        ...
        if (individualOnly && canUseIndividual) {
            if (isDispatch && profile?.user_id) {
                ...
                list = list.filter((t) => (t.driver1Id && set.has(t.driver1Id)) ||
        ...
        if (q) {
            list = list.filter((t) => t.truckNumber.toLowerCase().includes(q) ||
    ...
    useEffect(() => {
        if (search.trim() && filteredTrucks.length === 1) {
            const only = filteredTrucks[0].id;
            if (only !== selectedTruckId)
                setSelectedTruckId(only);
        }
    }, [search, filteredTrucks, selectedTruckId]);
    const loading = fleetLoading || locsLoading;
    return (<div className="flex h-[calc(100vh-3rem)] w-full overflow-hidden bg-background">
      {/* Sidebar list */}
      <aside className="flex w-80 flex-col border-r bg-card">
        <div className="border-b p-3">
          <div className="mb-2 flex items-center justify-between gap-2">
            ...
            {canUseIndividual && (<Button size="sm" variant={individualOnly ? "default" : "outline"} className="h-7 px-2 text-xs" onClick={() => setIndividualOnly((v) => !v)}>
                Individual
              </Button>)}
          </div>
          <div className="relative">
            <Search className="pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search truck # or driver" className="h-9 pl-8 text-sm"/>
          </div>
          <div className="mt-2">
            <Combobox options={[{ value: "", label: "All companies" }, ...companyOptions]} value={companyFilter} onValueChange={(v) => setCompanyFilter(v)} placeholder="Filter by company" searchPlaceholder="Search company..." emptyText="No companies found." className="h-9 text-sm"/>
          </div>
          {!isDispatch && (<div className="mt-2">
              <Combobox options={[
                { value: "", label: "All dispatchers" },
                ...dispatcherOptions,
            ]} value={dispatcherFilter} onValueChange={(v) => setDispatcherFilter(v)} placeholder="Filter by dispatcher" searchPlaceholder="Search dispatcher..." emptyText="No dispatchers found." className="h-9 text-sm"/>
            </div>)}
        ...
        <ScrollArea className="flex-1">
          <div className="divide-y">
            {filteredTrucks.length === 0 && !loading && (<div className="p-4 text-center text-xs text-muted-foreground">
                No trucks match this search.
              </div>)}
            {filteredTrucks.map((m) => (<div key={m.id} onClick={() => {
                setSearch(m.truckNumber);
                setSelectedTruckId(m.id);
            }} className={cn("flex cursor-pointer flex-col gap-0.5 px-3 py-2 text-sm hover:bg-accent", selectedTruckId === m.id && "bg-accent")}>
                <div className="flex w-full items-center justify-between gap-2">
                  <span className="font-semibold text-foreground">#{m.truckNumber}</span>
                  {m.currentOrder?.loadNumber && (<Badge variant="secondary" className="h-5 px-1.5 text-[10px] font-normal">
                ...
                {m.currentOrder?.brokerLoadNumber && (<span className="truncate text-[10px] text-muted-foreground">
      ...
      <div className="relative flex-1">
        <DispatcherFleetMapView trucks={filteredTrucks} singleHomeOnly pinnedPopup fullAddress flyToOnSelect externalSelectedTruckId={selectedTruckId} onTruckSelect={(id) => {
            const t = trucksWithData.find((x) => x.id === id);
            if (t)
                setSearch(t.truckNumber);
