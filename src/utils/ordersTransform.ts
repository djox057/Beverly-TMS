...
export function transformOrders(allOrders: any[]) {
  ...
  const transformed = (allOrders || []).map((order: any) => {
    ...
    return {
      ...
      weight: order.weight,
      weightRc: order.weight_rc ?? null,
      weightBol: order.weight_bol ?? null,
...
export function reverseTransformOrders(transformedOrders: any[]) {
  return (transformedOrders || []).map((order: any) => ({
    ...
    load_company_code: order.loadCompanyCode ?? null,
    ...
    commodity: order.commodity,
    weight: order.weight,
    weight_rc: order.weightRc ?? null,
    weight_bol: order.weightBol ?? null,
