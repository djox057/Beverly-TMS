...
export function YardActionComments({ actionId }: { actionId: string }) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    queryKey: ["yard-action-comments", actionId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("driver_yard_action_comments")
        .select("*")
        .eq("yard_action_id", actionId)
        .order("created_at", { ascending: true });
      if (error) throw error;
      return (data || []) as YardActionComment[];
    },
  });

  const refresh = () =>
    queryClient.invalidateQueries({ queryKey: ["yard-action-comments", actionId] });

  const handleAdd = async () => {
    const content = text.trim().slice(0, MAX_LENGTH);
    if (!content || !user) return;
    setSubmitting(true);
    const { error } = await supabase.from("driver_yard_action_comments").insert({
      yard_action_id: actionId,
      content,
      author_id: user.id,
...
export default YardActionComments;